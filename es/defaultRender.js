...
var defaultRenderText = function defaultRenderText(text, valueType, index, item) {
  ...
  if (valueType === 'dateRange' && text && Array.isArray(text) && text.length === 2) {
    // 值不存在的时候显示 "-"
    return React.createElement("div", null, React.createElement("div", null, text[0] ? moment(text[0]).format('YYYY-MM-DD') : '-'), React.createElement("div", null, text[1] ? moment(text[1]).format('YYYY-MM-DD') : '-'));
  }
  /**
   *如果是日期加时间类型的值
   */
  ...
  if (valueType === 'dateTimeRange' && text && Array.isArray(text) && text.length === 2) {
    // 值不存在的时候显示 "-"
    return React.createElement("div", null, React.createElement("div", null, text[0] ? moment(text[0]).format('YYYY-MM-DD HH:mm:ss') : '-'), React.createElement("div", null, text[1] ? moment(text[1]).format('YYYY-MM-DD HH:mm:ss') : '-'));
  }
  /**
   *如果是时间类型的值
   */
